...
class CodegenImplObject {
	...
	public static CodegenResult genObject(ClassInfo classInfo) {
		...
		ctx.append(String.format(
				"public static void encode_(%s obj, com.jsoniter.output.JsonStream stream) throws java.io.IOException {",
				classInfo.clazz.getCanonicalName()));
		if (hasFieldOutput(desc)) {
			...
			for (UnwrapperDescriptor unwrapper : desc.unwrappers) {
				if (unwrapper.isMap) {
					ctx.append(
							String.format("java.util.Map map = (java.util.Map)obj.%s();", unwrapper.method.getName()));
			...
			if (noIndention) {
				...
				} else if (notFirst == 2) { // // maybe not first, previous
											// field is omitNull
	...
	private static int genField(CodegenResult ctx, Binding binding, String toName, int notFirst) {
		...
		if (encoder == null) {
			...
			ctx.append(String.format("com.jsoniter.output.CodegenAccess.writeVal(\"%s\", %s, stream);", fieldCacheKey,
					valueAccessor));
		}
	...
	private static int appendComma(CodegenResult ctx, int notFirst) {
		...
		} else if (notFirst == 2) { // maybe not first, previous field is
									// omitNull
			if (noIndention) {
